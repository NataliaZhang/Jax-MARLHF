...
class MPEVisualizer(object):
    ...
    def init_render(self):
        ...
        for i in range(self.env.num_entities):
            ...
            # 以下为改动
            c_t = self.ax.text(state.p_pos[i][0], state.p_pos[i][1]+ 0.2, str(i),
                     ha='center', va='center')
            self.ax.add_artist(c_t)
            self.name_artists.append(c_t)
            # 以上为改动
    ...
    def update(self, frame):
        ...
        # 以下为改动
        for i, c_t in enumerate(self.name_artists):
            c_t.set_position([state.p_pos[i][0], state.p_pos[i][1]+0.2])
        # 以上为改动
