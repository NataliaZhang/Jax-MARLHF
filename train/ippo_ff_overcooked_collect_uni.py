...
def make_train(config):
    ...
    def train(rng):
        ...
        def _load_idiot_model(params):
            reward_model_path = config["IDIOT_PARAM_PATH"]
            with open(reward_model_path, "rb") as f:
                pretrain_params = serialization.from_bytes(params, f.read())
            return pretrain_params
        idiot_params = jax.experimental.io_callback(_load_idiot_model, network_params, network_params)
        ...
        def _batch_collect_step(runner_state, epoch):
            ...
            def _env_step(runner_state, unused):
                ...
                # get idiot actions
                pi_idiot, idiot_value = network.apply(idiot_params, obs_batch)
                action_idiot = pi_idiot.sample(seed=_rng)
                env_act_idiot = unbatchify(action_idiot, env.agents, config["NUM_ENVS"], env.num_agents)
                env_act_idiot = {k:v.flatten() for k,v in env_act_idiot.items()}
                # print(env_act_idiot.keys())
                # # breakpoint()
                # raise ValueError("Stop here")
                env_act['agent_0'] = env_act_idiot['agent_0']
