        ...
        def _update_step(runner_state, epoch):
            ...
            def _loss_fn(params, target_agent_params, init_hs, learn_traj):
                ...
                target_qvals = jax.tree.map(
                    lambda t_q, u: q_of_action(t_q, u.squeez())[1:], # get the greedy actions and avoid first timestep
                    target_q_vals,
                    learn_traj.actions
                )
                ...
                if config.get('TD_LAMBDA_LOSS', True):
                    ...
                    raise NotImplementedError("TD_LAMBDA_LOSS is not implemented yet")
                else:
                    # standard DQN loss
                    targets = (
            ...
            if config.get('VERBOSE', False):
                def callback(epoch, test_returns, predicted_returns, predicted_returns_with_log, params):
                    ...
                    if epoch == config["EPOCHS"] - 1:
                        ...
                        with open(f'{config["SAVE_PATH"]}/iql_model_{test_returns}.safetensors', 'wb') as f:
                            f.write(serialization.to_bytes(params))
                        print(f'Parameters of first batch saved in {config["SAVE_PATH"]}/iql_model_{test_returns}.safetensors')
                        # os.makedirs(config["SAVE_PATH"], exist_ok=True)
...
@hydra.main(version_base=None, config_path="config", config_name="iql_config")
def main(config):
    ...
    wandb.init(
        ...
        project=str(config["PROJECT_PREFIX"]) + config["env"]["ENV_NAME"].split("_")[2] + 'iql',
        tags=[
            alg_name.upper(),
            env_name.upper(),
            "RNN",
