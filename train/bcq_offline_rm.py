...
from utils.networks import ScannedRNN, AgentRNN, DoubleAgentRNN, AgentFF, ActorRNN, RewardModelFF, RewardModel, batchify, timestep_batchify, timestep_unbatchify, print_jnp_shapes
...
def make_train(config, env):
    ...
    def train(rng):
        ...
        # INIT NETWORK
        agent = DoubleAgentRNN(action_dim=wrapped_env.max_action_space, hidden_dim=config["AGENT_HIDDEN_DIM"], init_scale=config['AGENT_INIT_SCALE'])
        ...
        if config.get('PARAMETERS_SHARING', True):
            ...
            init_hs_1 = ScannedRNN.initialize_carry(1, config['AGENT_HIDDEN_DIM'], 0) # (batch_size, hidden_dim)
            init_hs = (init_hs_1, init_hs_1)
            network_params = agent.init(_rng, init_hs, init_x)
        else:
            raise NotImplementedError("Non-shared parameters are not implemented yet.")
            init_x = (
        ...
        # if config["IL_NUM_BATCH"] is default:
        if (config["USE_FIX_IL"]):
            ref_model_path = config["REFERENCE_NETWORK_PARAM_PATH"] + "|".join([str(r) for r in config["IL_NUM_BATCH"]]) + 'final.msgpack'
        else:
        ...
        def _update_step(runner_state, epoch):
            ...
            if config.get('PARAMETERS_SHARING', True):
                init_hs_1 = ScannedRNN.initialize_carry(env.num_agents*config["BUFFER_BATCH_SIZE"], config['AGENT_HIDDEN_DIM'], 1) # (n_agents*batch_size, hs_size)
                init_hs_2 = ScannedRNN.initialize_carry(env.num_agents*config["BUFFER_BATCH_SIZE"], config['AGENT_HIDDEN_DIM'], 2) # (n_agents*batch_size, hs_size)
                init_hs = (init_hs_1, init_hs_2)
            else:
                raise NotImplementedError("Non-shared parameters are not implemented yet.")
        ...
        def get_greedy_metrics(rng, params, time_state, epoch):
            ...
            if config["PARAMETERS_SHARING"]:
                hstate_1 = ScannedRNN.initialize_carry(env.num_agents*config["NUM_TEST_EPISODES"], config['AGENT_HIDDEN_DIM'], 1) # (n_agents*n_envs, hs_size)
                hstate_2 = ScannedRNN.initialize_carry(env.num_agents*config["NUM_TEST_EPISODES"], config['AGENT_HIDDEN_DIM'], 2) # (n_agents*n_envs, hs_size)
                hstate = (hstate_1, hstate_2)
            else:
                raise NotImplementedError("Non-shared parameters are not implemented yet.")
            ...
            if config.get('VERBOSE', False):
                def callback(epoch, test_returns, predicted_returns, predicted_returns_with_log, params):
                    ...
                    if epoch == config["EPOCHS"] - 1:
                        ...
                        # os.makedirs(config["SAVE_PATH"], exist_ok=True)
                        # with open(f'{config["SAVE_PATH"]}/vdn_ff_model_{test_returns}.safetensors', 'wb') as f:
                        #     f.write(serialization.to_bytes(params))
                        # print(f'Parameters of first batch saved in {config["SAVE_PATH"]}/vdn_ff_model_{test_returns}.safetensors')
...
def main(config):
    ...
    alg_name = f'bcq_{"ps" if config["alg"].get("PARAMETERS_SHARING", True) else "ns"}'
    ...
    wandb.init(
        ...
        project=str(config["PROJECT_PREFIX"]) + config["env"]["ENV_NAME"].split("_")[2] + 'bcq',
        tags=[
            alg_name.upper(),
            env_name.upper(),
            "RNN",
